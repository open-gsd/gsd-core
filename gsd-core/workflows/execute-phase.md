...
`USE_WORKTREES` and `ISOLATION` are also reset for the run when `worktree base-check` detects the orchestrator HEAD has diverged from the worktree fork base (#683; e.g. an unmerged milestone branch). This runs for **any** isolated run, not only Claude: fork-base divergence is a repository property, so it degrades GSD-created and harness-created worktrees alike. The auto-degrade prints a one-line stderr warning and falls through to the sequential path so executors do not hit the exit-42 halt. Setting `worktree.baseRef:"head"` restores parallel execution where GSD creates the worktrees (orchestrator-managed runtimes: Codex, OpenCode, Kimi, Kimi Code), and on harness-isolated runtimes (Claude Code, Cursor) only from the **user/global** layer (`/config`; #1013): those harnesses ignore **project**-settings `baseRef` (#48, 5/5; upstream claude-code#44965), so a project-layer `"head"` still compares against the real fork base; parallel execution returns once HEAD is merged/pushed so `origin/HEAD` matches it (#3659, #4090). The `worktree-branch-check` exit-42 guard inside each executor remains a backstop.
