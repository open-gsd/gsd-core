...
**AI integration capability:** If the active `ai-integration` step hook is present, `AI_SPEC_PATH` is empty, and the phase goal contains at least one keyword from this list: `agent`, `llm`, `rag`, `chatbot`, `embedding`, `langchain`, `llamaindex`, `crewai`, `langgraph`, `openai`, `anthropic`, `vector`, `eval`, `model evaluation`, `evaluation harness`, `ai system` — matched as whole words or whole phrases, where plural forms count (`agents`, `LLMs`, `evals` all match) but a keyword embedded inside a longer word does not (`eval` is a substring of `evaluation` and `retrieval`, and `rag` of `storage`, so none of those goals match on that keyword alone), then:
