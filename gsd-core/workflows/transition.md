...
VERIFY_STATUS=$(printf '%s' "$VERIFY_JSON" | jq -r '.status // empty' 2>/dev/null || echo "")
```
...
unless canonical `*-VERIFICATION.md` frontmatter `status` is `passed` and no
summary file is newer than the verification file (stale verification is also
rejected even when frontmatter says `passed`).
