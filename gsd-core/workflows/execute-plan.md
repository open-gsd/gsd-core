...
`ISOLATION` — not `RUNTIME` — gates the Pattern A worktree decision below. Substitute
`{harnessFlag}` in Pattern A's `Agent()` with `$HARNESS_FLAG`+comma when
`ISOLATION = "harness-worktree"`, else empty. `{harnessFlag}`
