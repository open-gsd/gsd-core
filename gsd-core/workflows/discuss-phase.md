...
```bash
DISCUSS_PRE_HOOKS_JSON=$(gsd_run loop render-hooks discuss:pre --raw --phase "${PHASE_NUMBER}")
```
...
```bash
DISCUSS_POST_HOOKS_JSON=$(gsd_run loop render-hooks discuss:post --raw --phase "${PHASE_NUMBER}")
```
