...
**`verify-phase.md` is never entered directly (#2529):** the `verify_phase_goal` step reaches it only by spawning a `gsd-verifier` subagent, so the workflow file itself has no place to carry a directive. The dispatch prompt built in that step MUST carry this line verbatim, immediately after `Create VERIFICATION.md.`:
