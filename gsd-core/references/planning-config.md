...
| `workflow.session_outlives_turn` | `true` | When `true` (default), executor and verifier agents in `execute-phase` are dispatched asynchronously in the background. When `false`, executor and verifier agents are dispatched synchronously in the foreground (`run_in_background: false`) and awaited sequentially before proceeding (multi-plan waves run sequentially). Designed for wrapper hosts whose orchestrator session terminates with the active turn. Does not affect tool availability, runtime selection, git worktree isolation, or standalone workflows outside `execute-phase`. |
...
| `workflow.session_outlives_turn` | boolean | `true` | `true`, `false` | When `false`, dispatches executor and verifier agents in `execute-phase` in the foreground and awaits each sequentially before proceeding. Does not affect tool availability, runtime selection, git worktree isolation, or standalone workflows outside `execute-phase`. |
