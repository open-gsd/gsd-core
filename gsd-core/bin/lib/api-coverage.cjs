...
exports._isCoverageHeader = _isCoverageHeader;
 ...
 */
function _isCoverageHeader(cells) {
    return isCoverageHeader(cells);
}
function isCoverageHeader(cells) {
    ...
    const m = (0, markdown_table_cjs_1.matchTableSchema)(cells, { caseInsensitive: true });
 ...
 */
function isStrongDelimiterRow(cells) {
...
function peelCodeSpan(s) {
    ...
    if (open === 0)
        return null;
    let close = 0;
    while (close < s.length && s[s.length - 1 - close] === '`')
        close += 1;
    // Equal-length runs, with at least one non-backtick character between them.
    if (open !== close || open + close >= s.length)
        return null;
 ...
 */
function stripInlineEmphasis(cell) {
    let s = cell.trim();
    if (s.length > DECISION_NORMALIZE_MAX)
        return s;
    for (;;) {
        ...
        for (const mark of ['**', '__', '*', '_']) {
            if (s.length <= 2 * mark.length || !s.startsWith(mark) || !s.endsWith(mark))
                continue;
...
function parseCoverageMatrix(text) {
    ...
    for (const b of (0, markdown_sectionizer_cjs_1.scanFencedBlocks)(lines)) {
        ...
        if (b.closeLineIdx === -1) {
            out.errors.push(`fence: unterminated code fence opened at line ${b.openLineIdx + 1} `
    ...
    const reportLooseRow = (cells) => {
        ...
        out.errors.push(`row: coverage row ${JSON.stringify(cells[0] || '')} appears outside a coverage table `
    ...
    const reportNearMissHeader = (cells) => {
        const lower = cells.map((c) => c.toLowerCase().trim());
        if (!lower.includes('capability') || !lower.includes('decision'))
            return;
        out.errors.push(`table: found a table headed ${JSON.stringify(cells.join(' | '))} — not the canonical `
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        if (state === 'foreignHeader?') {
            if ((0, markdown_table_cjs_1.isDelimiterRow)(cells)) {
                if (heldRow !== null)
                    reportNearMissHeader(heldRow);
                heldRow = null;
                state = 'foreignRows';
            }
            else {
        ...
        if (state === 'expectDelimiter') {
            ...
            if (!(0, markdown_table_cjs_1.isDelimiterRow)(cells)) {
                ...
                out.errors.push(`row: delimiter row has ${cells.length} columns, expected ${headerCols}`);
                state = 'seeking';
            }
            else {
                state = 'inRows';
        ...
        if (cells.length !== headerCols) {
            out.errors.push(`row: ${cells.length} columns (expected ${headerCols}: capability | decision | reason)`);
            continue;
        }
...
function renderCoverageMatrix(rows) {
    ...
    const body = rows
        .map((r) => `| ${(0, markdown_table_cjs_1.escapeCell)(r.capability)} | ${r.decision} | ${(0, markdown_table_cjs_1.escapeCell)(r.reason)} |`)
        .join('\n');
