...
#      the exact `gsd-tools query commit` / `gsd_run query commit` command
#      shapes; those commands invoke git internally, so the literal "git commit"
#      substring never appears — see #3653)
#   3. $CI is unset/empty
#   4. Inside a git repo
...
case "$COMMAND" in
  ...
  *"gsd_run query commit"|*"gsd_run query commit "*) ;;
  *) exit 0 ;;
esac
