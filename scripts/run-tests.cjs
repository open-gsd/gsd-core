...
function makeFileWeigher(timings) {
  if (!timings) return () => 1;
  return (f) => {
    const ms = timings.timings[basename(f)];
...
function packChunks(files, { weightOf, maxWeight, maxChars, fixedOverhead }) {
  ...
  let chunkCount = Math.max(
    1,
    Math.ceil(totalWeight / maxWeight),
    Math.ceil(files.length / maxWeight),
  );
  for (;;) {
    ...
    for (const entry of heaviestFirst) {
      ...
      for (const bin of bins) {
        ...
        if (bin.entries.length > 0 && bin.chars + entry.chars > maxChars) continue;
