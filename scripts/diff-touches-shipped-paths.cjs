#!/usr/bin/env node
/**
 * Used by the hotfix cherry-pick loop to decide whether a candidate commit
 * can possibly change what ships in the npm package.
 *
 ...
 *   - CI-gating test paths under `tests/` — these don't ship in the tarball,
 *     but they gate the hotfix-branch test job. A test fixture update that
 *     aligns with a cherry-picked production fix MUST be pickable or CI fails
 *     on the hotfix run.
