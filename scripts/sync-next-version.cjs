...
function syncViaPr(version, { run = defaultRun } = {}) {
  ...
  run('git', ['add', '-A'], { timeout: 15000 });
  // `git diff --cached --quiet` exits 0 when there is NO staged diff (execFileSync
  // returns), non-zero when there IS one (execFileSync throws). Proceed only on a diff.
  let hasDiff = false;
  try {
  ...
  const prUrl = run('gh', ['pr', 'create', '--base', 'next', '--head', branch,
