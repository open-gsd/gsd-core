...
describe('MiMoCode in runtime selection', () => {
  test('--mimo flag is recognized in parseRuntimeInput', () => {
    const result = parseRuntimeInput('12');
    assert.deepStrictEqual(result, ['mimo']);
  });

  test('--all includes mimo', () => {
    assert.ok(allRuntimes.includes('mimo'), 'allRuntimes should include mimo');
  });

  test('runtimeMap maps 12 to mimo', () => {
    assert.strictEqual(runtimeMap['12'], 'mimo');
  });
