...
describe('multi-runtime selection parsing', () => {
  ...
  test('choice 17 returns Oh My Pi', () => {
...
describe('install.js exports multi-select runtime metadata', () => {
  ...
  test('--omp and --pi both select the canonical omp runtime', () => {
    assert.deepStrictEqual(selectRuntimesFromArgs(['--omp']), ['omp']);
    assert.deepStrictEqual(selectRuntimesFromArgs(['--pi']), ['omp']);
    assert.deepStrictEqual(selectRuntimesFromArgs(['--omp', '--pi']), ['omp']);
  });

  test('prompt lists Hermes Agent (10), Kimi (11), Qwen Code (14), Trae (15), Oh My Pi (17), and All (18)', () => {
    ...
    assert.ok(/\b17\)\s*Oh My Pi\b/.test(prompt),
      'prompt lists Oh My Pi as option 17');
