  ...
  test(`install tree — ${runtime}`, async (t) => {
    ...
    if (!fs.existsSync(fixturePath)) {
      assert.fail(
        ...
        'Run UPDATE_INSTALL_TREE=1 node --test tests/golden-install-tree.test.cjs to capture.'
      );
    }
    ...
    if (added.length > 0 || removed.length > 0) {
      ...
      lines.push('Run UPDATE_INSTALL_TREE=1 to recapture if the change is intentional.');
