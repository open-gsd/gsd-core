 ...
 *
 * This asserts the ADR-457 migration end state for all nine: none tracked and
 * all gitignored. The regime-agnostic sync guard (added in #2656,
 * scripts/lint-compiled-artifact-sync.cjs) permits no unowned tracked direct
 * tsc output. Outputs generated by the OpenCode V2 bundle producer are instead
 * covered by that producer's own exact output manifest and sync check.
 *
...
} = require('../scripts/lint-compiled-artifact-sync.cjs');
const { getBundleArtifactManifest } = require('../scripts/build-opencode-v2-bundles.cjs');
...
const V2_DIRECT_TSC_ARTIFACTS = [
  'journal-lock.cjs',
  'quick-batch-v2.cjs',
  'quick-batch-v2-command-router.cjs',
  'opencode-v2-verification-receipt.cjs',
  'opencode-v2-worktree-command-router.cjs',
  'opencode-v2-worktree-mutation.cjs',
  'opencode-v2-worktree-provisioner.cjs',
].map((name) => ({
  artifact: `${LIB_DIR}/${name}`,
  source: `src/${path.basename(name, '.cjs')}.cts`,
}));

const V2_BUNDLE_ATTESTATION_ARTIFACT = {
  artifact: `${LIB_DIR}/opencode-v2-attestation.cjs`,
  source: 'src/opencode-v2-attestation.cts',
};
...
describe('fix-2657: compiled .cjs artifacts are gitignored, not tracked (ADR-457)', () => {
  ...
  test('trackedCompiledArtifacts() excludes all nine ADR-457 migration-gap artifacts', () => {
    let pairs;
    try {
  ...
  test('lint-compiled-artifact-sync exits 0 with no tracked direct tsc output', () => {
    // #4093 CI: this spawn is NOT the PROBE class the shared default below
    // describes. The script's no-artifact path must short-circuit before it
    // runs a full `tsc -p tsconfig.build.json` compile. Under CI shard load that compile can
    // exceed even a generous budget, dying to a SIGTERM with empty piped
    // stdout (observed on ubuntu shard 1/3 both pre- and post-#4488). Per
    // helpers/timeouts.cjs's own rule, this conditional compile keeps its own
    // local constant; see tests/ensure-runtime-build.test.cjs's
    // BUILD_TIMEOUT_MS for the other instance.
    //
    // The OpenCode V2 bundle has a separate exact sync gate; it is not a
    // direct-tsc artifact covered by this guard.
  ...
  test('trackedCompiledArtifacts() reports the fully-empty ADR-457 end state', () => {
    let pairs;
    try {
      pairs = trackedCompiledArtifacts();
    } catch (err) {
...
describe('compiled artifact producer ownership', () => {
  const ordinaryArtifact = 'gsd-core/bin/lib/ordinary.cjs';
  const ordinarySource = 'src/ordinary.cts';

  test('classifies the complete V2 CJS set: seven direct tsc outputs and bundle-owned attestation', () => {
    const bundleManifest = getBundleArtifactManifest();
    ...
    const allV2Sources = new Set([
      ...V2_DIRECT_TSC_ARTIFACTS.map(({ source }) => source),
      V2_BUNDLE_ATTESTATION_ARTIFACT.source,
    ]);
    const classification = classifyTrackedCompiledArtifacts({
      tracked: [
        ...V2_DIRECT_TSC_ARTIFACTS.map(({ artifact }) => artifact),
        V2_BUNDLE_ATTESTATION_ARTIFACT.artifact,
        ordinaryArtifact,
      ],
      sourceExists: (source) => allV2Sources.has(source) || source === ordinarySource,
      producerManifests: [bundleManifest],
    });

    assert.deepEqual(classification.pairs, [
      ...V2_DIRECT_TSC_ARTIFACTS,
      { artifact: ordinaryArtifact, source: ordinarySource },
    ].sort((a, b) => a.artifact.localeCompare(b.artifact)));
    assert.deepEqual(classification.producerOwned, [{
      artifact: V2_BUNDLE_ATTESTATION_ARTIFACT.artifact,
      source: V2_BUNDLE_ATTESTATION_ARTIFACT.source,
      producer: 'opencode-v2-bundles',
    }]);
    assert.ok(
      bundleManifest.outputs.includes(V2_BUNDLE_ATTESTATION_ARTIFACT.artifact),
      'the excluded tsc pair must remain owned and byte-checked by the bundle producer manifest',
    );
    const packageScripts = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8')).scripts;
    assert.match(packageScripts['check:opencode-v2-bundles'], /build-opencode-v2-bundles\.cjs --check/);
  });

  test('requiring bundle producer metadata has no build or write side effect', () => {
    const script = path.join(REPO_ROOT, 'scripts', 'build-opencode-v2-bundles.cjs');
    const result = run(process.execPath, ['-e', `require(${JSON.stringify(script)}).getBundleArtifactManifest()`]);
    assert.equal(result.status, 0, describeFailure(process.execPath, [script], result));
    assert.equal(result.stdout, '');
    assert.equal(result.stderr, '');
  });

  test('normal direct tsc artifact drift remains detected', () => {
    const root = createTempDir('gsd-artifact-policy-');
    const buildDir = path.join(root, 'emit');
    const artifact = path.join(root, ordinaryArtifact);
    const emitted = path.join(buildDir, 'ordinary.cjs');
    try {
      fs.mkdirSync(path.dirname(artifact), { recursive: true });
      fs.mkdirSync(path.dirname(emitted), { recursive: true });
      fs.writeFileSync(artifact, 'committed\n');
      fs.writeFileSync(emitted, 'fresh compile\n');
      assert.deepEqual(
        compareCompiledArtifacts([{ artifact: ordinaryArtifact, source: ordinarySource }], buildDir, root),
        {
          drifted: [{
            artifact: ordinaryArtifact,
            source: ordinarySource,
            committed: Buffer.byteLength('committed\n'),
            expected: Buffer.byteLength('fresh compile\n'),
          }],
          missing: [],
        },
      );
  ...
  test('producer ownership metadata fails closed on ambiguity, invalid paths, and source collisions', () => {
    assert.throws(
      () => producerOwnedOutputs([
        { producer: 'one', outputs: [ordinaryArtifact] },
        { producer: 'two', outputs: [ordinaryArtifact] },
      ]),
      /ambiguous producer ownership/,
    );
    assert.throws(
      () => producerOwnedOutputs([{ producer: 'outside', outputs: ['../ordinary.cjs'] }]),
      /escapes the repository/,
    );
    assert.throws(
      () => classifyTrackedCompiledArtifacts({
        tracked: [ordinaryArtifact],
        sourceExists: (source) => source === ordinarySource,
        producerManifests: [{ producer: 'collision', outputs: [ordinarySource] }],
      }),
