...
| `gsd-write-guard.js` | `PreToolUse` | キュレーションされた `.planning/` アーティファクト（ROADMAP.md、マイルストーンロードマップ、STATE.md）を大幅に縮小するファイル全体の `Write` をハードブロック。`GSD_ALLOW_PLANNING_SHRINK=1` でオーバーライド（#2255、#973 の修正 3） |
