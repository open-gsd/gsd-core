...
| `worktree-path-safety.md` | エグゼキューターのパスガード：ディスパッチ時に埋め込むルート pin（ステップ 0p、#4254）、保護/デフォルトブランチの HEAD（ステップ 0、#3819）、cwd ドリフト（ステップ 0a、#3097）、絶対パス境界（ステップ 0b、#3099）。ルートの正規化と固定された親リポジトリのサブモジュール規約を含む。 |
