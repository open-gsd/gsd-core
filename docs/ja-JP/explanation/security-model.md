...
**ランタイムフック：`gsd-read-injection-scanner.js`。** このフックはすべての Read、WebFetch、および WebSearch ツール呼び出しの出力で発火します。GSD がエージェントのコンテキストに組み込もうとしているファイルまたはリモートリソースの *読み取ったばかりのコンテンツ* をスキャンし、攻撃者が命令を埋め込んでいるケースをキャッチします。8 つの調査および文書取り込みエージェントはさらに、共有の `<security_context>` データ/命令境界（`gsd-core/references/untrusted-input-boundary.md` で定義）を持ちます：それらのエージェントが取得または読み取ったコンテンツは、コンテンツが何を主張していても、指示としてではなくデータとして扱われます。

**オプトインブロッキング（`security.injection_blocking`）。** デフォルトでは、すべてのインジェクション検出はアドバイザリーのみです（ログに記録されますが、ブロックされません）。`.planning/config.json` で `security.injection_blocking = true` を設定すると、HIGH 信頼度の検出が**ブロッキング**にアップグレードされます：フックは Read、WebFetch、または WebSearch の結果を拒否し、コンテンツがエージェントコンテキストに入る前に人間のレビューのために発見を表示します。LOW および MEDIUM 検出はこの設定でもアドバイザリーのままです。このフラグはオプトインです；デフォルト（アドバイザリーのみ）は既存のワークフローを壊さないために維持されます。
...
2 つのフックは補完的な面をカバーします。`gsd-prompt-guard.js` は *計画アーティファクトへの書き込み* を監視します——植え付けられているインジェクションをキャッチします。`gsd-read-injection-scanner.js` は *任意のファイルの読み取りおよびリモートフェッチ* を監視します——外部コンテンツ（依存関係の README、サードパーティの設定ファイル、ユーザー提供のドキュメント、または WebFetch や WebSearch 経由でフェッチされた URL）から取り込まれるインジェクションをキャッチします。調査エージェント内のプロンプト内 `<security_context>` 境界は追加の封じ込めレイヤーを提供します：インジェクションされた文字列がエージェントに届いた場合でも、指示領域から構造的に分離されています。合わせて、取り込み → 保存 → 再読み取りのライフサイクルを括ります。
...
**プロンプトインジェクション防御が排除しないもの：** 既知のパターンにマッチしない十分に創造的なインジェクション、またはフックがカバーしないチャンネルを通じて届くインジェクション。依存関係の公開 README にインジェクトされたコンテンツがサブエージェントによって読み取られるという以前はカバーされていなかったチャンネルは、現在 `gsd-read-injection-scanner.js`（WebFetch および WebSearch の出力をカバー）によって取り込み時にスキャンされ、調査エージェントの `<security_context>` 境界によってプロンプト内で構造的に分離されています——ただし、新しいジェイルブレイクや低シグナルのインジェクションは検出されない可能性があります。多層防御は各レイヤーが攻撃を困難にすることを意味し、単一のレイヤーが不可能にすることを意味しません。
