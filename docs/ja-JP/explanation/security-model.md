...
**オプトインブロッキング（`security.injection_blocking`）。** デフォルトでは、すべてのインジェクション検出はアドバイザリーのみです（ログに記録されますが、ブロックされません）。`.planning/config.json` で `security.injection_blocking = true` を設定すると、HIGH 信頼度の検出が**ブロッキング**にアップグレードされます：フックは Read、WebFetch、または WebSearch の結果を拒否し、エージェントが検出されたコンテンツを処理または永続化しないようにするブロッキングシグナルを通知します。LOW 検出はこの設定でもアドバイザリーのままです。このフラグはオプトインです；デフォルト（アドバイザリーのみ）は既存のワークフローを壊さないために維持されます。
