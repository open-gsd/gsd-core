...
| `context_window` | number | any integer | `200000` | Context window size in tokens. Set `1000000` for 1M-context models. Values `>= 500000` enable adaptive context enrichment (full-body reads of prior SUMMARY.md, deeper anti-pattern reads). Configured via `/gsd-config --advanced`. |
...
Known providers: `openai`, `anthropic`, `google`, `qwen`. Budget levels: `high`, `medium`, `low`. Use `anthropic` to keep the Opus 4.8-backed Claude preset. On the default `claude` runtime, policy-resolved model IDs are mapped to Claude Code agent aliases; an ID with no corresponding Claude alias emits a warning and falls back to the configured tier.
