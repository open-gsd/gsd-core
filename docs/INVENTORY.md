...
| `yaml-anchor-guard.cjs` | Single owner of ADR-3473 §8.1's YAML anchor/alias refusal (compiled from `src/yaml-anchor-guard.cts`, gitignored) — exports `AnchorDetectedSignal` and the js-yaml `listener` `refuseAnchorsListener`, which throws from *inside* the listener so the parse aborts before any alias expansion (a listener that only records a flag lets js-yaml keep expanding, reopening the resource-exhaustion window the guard exists to close). `FAILSAFE_SCHEMA` restricts tag resolution only and does not disable anchors, so both YAML entry points — `frontmatter.cjs`'s `refuseAnchorsAndAliases` pre-pass and `shell-command-projection.cjs`'s `_normalizeMd` frontmatter classifier — share this one implementation instead of each carrying a copy that can silently drift (#4499) |
