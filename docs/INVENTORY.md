...
| `journal-lock.cjs` | Crash-visible, non-waiting bakery journal lock for the V2 durable worktree journal (OpenCode V2-only native transport) — each contender owns immutable `owner-<token>.json`/`ticket-<token>.json` records published via `linkSync` (never a replaceable singleton), release is owner-first so an orphaned ticket is reapeable, staleness classification uses exact boot-id + process-start-id identity probes instead of leases/timeouts, and every operation requires a positively identified local filesystem (explicit `statfs` f_type allowlist) plus symlink-free canonical path components; also exports `durableReplaceJson` (write+fsync+rename+dir-fsync) for the journal's durable file replacement. Compiled from `src/journal-lock.cts` |
...
| `quick-batch-command-router.cjs` | Thin CJS subcommand router for `gsd-tools quick-batch` (#3676, Phase 4 of epic #3344 / ADR-1239 "Quick-batch binding"; compiled from `src/quick-batch-command-router.cts`, gitignored) — a first-party, always-on command family wired directly into `HOST_COMMAND_ROUTERS` (like `state`/`phase`), NOT the opt-in capability-registry/`activationKey` path `graphify` uses. In addition to the established batch and scheduling verbs, its OpenCode V2 surface provides strict coordinate-only `v2-attest`/`v2-verify` plus exact-field `v2-merge`/`v2-teardown`/`v2-complete`; unknown, duplicate, alias, positional, or omitted authorization fields fail closed before reaching journal operations, and generic completion refuses an active native item. |
| `opencode-v2-attestation.cjs` | Generated, dependency-bundled OpenCode V2 trusted attestation client (built from `src/opencode-v2-attestation.cts` by `scripts/build-opencode-v2-bundles.cjs`, with a source map and third-party notice) — discovers only a compatible managed OpenCode service, health-checks it, and calls the registered `gsd-worktree-task.attestation.v1` status RPC with parent/wave coordinates. It accepts no caller-supplied status evidence, endpoint, headers, or observer hook and returns bounded RPC provenance for the journal gate without relying on an installed `node_modules` tree. |
| `opencode-v2-verification-receipt.cjs` | OpenCode V2 canonical verification-receipt policy — validates the exact quick-item verification artifact, its canonical `status`, staleness inputs, and SHA-256 binding before a V2 completion path can authorize the outcome. Compiled from `src/opencode-v2-verification-receipt.cts`. |
| `opencode-v2-worktree-command-router.cjs` | Strict OpenCode V2 `worktree merge-one`/`teardown-one` CLI router — parses exact identity flags and rejects direct native mutations unless the V2 quick-batch authorization route owns them. Compiled from `src/opencode-v2-worktree-command-router.cts`. |
| `opencode-v2-worktree-mutation.cjs` | OpenCode V2 durable worktree Git-mutation core — validates a manifest-bound worktree identity and performs authorized merge or teardown through injected generic primitives. Compiled from `src/opencode-v2-worktree-mutation.cts`. |
| `opencode-v2-worktree-provisioner.cjs` | OpenCode V2 linked-worktree provisioner — after creation, copies the source `.opencode` tree when present and rejects a destination inside that source tree. Compiled from `src/opencode-v2-worktree-provisioner.cts`. |
...
| `quick-batch-v2-command-router.cjs` | Strict coordinate-bound `gsd-tools quick-batch v2-*` router — dispatches V2 lifecycle, attestation, verification, merge, teardown, completion, and cleanup verbs while rejecting unknown, duplicate, alias, positional, or omitted authorization fields. Compiled from `src/quick-batch-v2-command-router.cts`. |
| `quick-batch-v2.cjs` | Durable V2 journal and mutation authorization boundary for one parent-session/batch pair of the OpenCode V2-only native worktree transport (compiled from `src/quick-batch-v2.cts`, gitignored). It owns native transport/runtime metadata, optimistic revisions, two-stage ≤30 s RPC attestation, exact `merge_intent` identity/tip/digest checks, and authorized merge/teardown calls under the journal lock. Validation-enabled rounds bind a helper-read canonical verification artifact path/status/SHA-256 receipt; generic completion cannot create it, while `completed` and close reread both receipt and BATCH outcome. Recoverable policy/Git capability and sync-pending results remain nonterminal and preserve resources. |
