...
| `git-scope-commits.cjs` | Commit-subject scope selector (`gsd_run query git scope-commits --phase <id> \| --plan <id> --range <rev-range>`) for `/gsd-undo` (#4661): prints the commits in a range whose conventional-commit header DECLARES the phase or plan as its scope. Pins the git record shape (`--no-color`, explicit `--format`), unwraps `fixup!`/`squash!`/`amend!`/`Revert "`/`Reapply "` subjects, extracts the scope through the one header matcher in `scripts/release-notes/conventional-title.cjs` (shipped by the installer for this module), and compares ids as values — `comparePhaseNum` behind a strict shape check, a literal string otherwise — so the id is never interpolated into a pattern |
