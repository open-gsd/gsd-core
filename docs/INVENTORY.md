...
| `health-diagnostic-rules/project-skills-index.cjs` | Health-diagnostic rule: project skill index check (W030) — flags a project skill whose SKILL.md has no frontmatter `description` (project-skills discovery then reads it in full on every discovery-agent spawn) or exceeds the Agent Skills 500-line SKILL.md guideline; scans the skill discovery contract's project roots through `init.cjs`'s `buildSkillManifest`, skips GSD's own `gsd-*` skills, is WARNING-severity and never auto-fixable, and degrades to `[]` when the roots cannot be scanned (#4649) |
