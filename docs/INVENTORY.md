...
| `agent-tools-contract.cjs` | Single parsing seam for an agent's declared `tools:` contract (#2540) — `parseToolsContract` (inline, block-list, and flow YAML shapes) + `toolsRequireWrite`; consumed by the Codex sandbox derivation, the Codex agent converter, and the `validate agents` semantic sandbox check |
