...
| `claude-orchestration.cjs` | Claude orchestration policy module (#1143) - pure resolver plus CLI/check handlers for the default-off `claude-orchestration` capability; selects `inline` vs `workflow` backend and exposes the uniform pre-wave gate contract |
