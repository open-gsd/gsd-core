...
| `planner-verify-command-grounding.md` | Verify Command Grounding rules (#2401): inherit `prior_verify_commands` verbatim when the story repeats, prefer `npm --prefix <dir> run <script>` over `cd <dir> && npm run <script>`, and ground every authored path. Also the root-relative path-form rule (#4767): every path in the plan body is repo-root-relative and every `<automated>` command assumes cwd at the checkout root; only `<required_reading>` inputs are absolute (#2376), and a harvested command carrying an absolute path is re-rooted before reuse. |
...
| `tdd-red-evidence.cjs` | Format-independent RED-evidence policy (#3770, #4692, compiled from `src/tdd-red-evidence.cts`): requires a complete supported report and a matching target failure, preserves the matching test in the evidence record, and rejects ambiguous targets. |
| `report-parser.cjs` | Test-report adapters (#4692, compiled from `src/report-parser.cts`): normalize Node/Vitest TAP and Surefire/Failsafe JUnit XML into individual results with identities, groups, statuses, and report validity. |
...
| `vendor/tap-parser.cjs` | Reproducibly bundled upstream TAP parser and locked dependencies, with companion license notices. Shared by Node and Vitest RED evidence; no installed node_modules required. |
| `vendor/saxes.cjs` | Reproducibly bundled upstream XML parser and locked dependencies, with companion license notices. Used by the JUnit report adapter; no installed node_modules required. |
