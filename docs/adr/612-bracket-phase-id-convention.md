...
2. **M-NN is a shipped, first-class convention** with live machinery: a deprecation-style migration nudge in `src/roadmap-parser.cts:361` (`getMilestonePhaseFilter`, warning body at `:372-376`) and the W021 read-path check in `src/verify.cts:1723` (`phase_id_convention === 'milestone-prefixed'` branch, warning at `:1732`). W021 is pinned by shipped `tests/milestone-prefixed-convention.test.cjs` and cannot be renumbered.
...
The pair is verified with **`fast-check` property tests** (`fast-check ^4.8.0` is an installed devDependency on this base — `package.json:64` — and `tests/phase-id.test.cjs` already imports it), holding the bijective contract the maintainer requires:
...
- The deprecation is grounded in machinery that already exists: the `getMilestonePhaseFilter` nudge (`src/roadmap-parser.cts:372-376`) and the W021 hint (`src/verify.cts:1732`) already tell M-NN repos to run the migrator. The bracket migrator (Decision 7) extends that forward path to carry M-NN → bracket.
