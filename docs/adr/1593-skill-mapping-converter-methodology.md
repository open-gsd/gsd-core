...
| 3 | **Tool-name rewrites** | Map Claude tool names to runtime equivalents. | `convertToolName`, `convertKimiToolName`, `convertCopilotToolName`, `convertAntigravityToolName`; inline: `AskUserQuestion`→`question`, `SlashCommand`→`skill` (opencode) |
