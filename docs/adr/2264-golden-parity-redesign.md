...
2. **Anti-staleness CI selection** — `scripts/ci-test-scope.cjs` gains a rule selecting `golden-install-parity` + the file-set snapshot whenever ANY installed-source path changes (`gsd-core/**`, `hooks/**`, `commands/**`, `agents/**`, `skills/**`, shipped `scripts/*`), not just `src/`/installer paths. This makes the #2266 silent-staleness class impossible: a source edit that changes emitted output re-verifies the fixtures at PR time.
