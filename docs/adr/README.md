...
- **A `Proposed` status is not a claim that the work is unbuilt.** Several ADRs here are `Proposed` while the decision they describe has shipped; each such file now carries a dated *status caveat* saying so. Trust the caveat over the label, and see [Ratifying a stale `Proposed`](#ratifying-a-stale-proposed).
...
| `Proposed` | Decided in principle, not ratified. Do not cite as settled. | If the work has shipped, add a dated status caveat. |
...
When an ADR says `Proposed` but the work has shipped, the fix is a maintainer act, not a cleanup: change the status to `Accepted`, then run `node scripts/gen-adr-index.cjs --write`. If that ADR claims to supersede others, the check will now require their back-links — which is the point. Ratify deliberately.
...
| [ADR-0011](0011-skill-surface-budget-module.md) | Skill Surface Budget Module owns install-time profile staging and runtime surface control | Accepted | — |
| [ADR-58](58-runtime-install-policy-module.md) | Runtime Install Policy Module owns the typed install-plan projection | Accepted | [ADR-1239](1239-gsd-embeddable-orchestration-engine.md) |
...
| [ADR-857](857-capability-system.md) | Capability system — five-step loop as core, features as plug-ins behind Loop Extension Points | Proposed | — |
| [ADR-894](894-capability-declaration-format.md) | Capability declaration format + registry generation | Proposed | [ADR-1239](1239-gsd-embeddable-orchestration-engine.md) |
...
| [ADR-1244](1244-capability-ecosystem.md) | Capability Ecosystem: third-party authoring, versioned manifests, and URL import/upgrade/remove | Proposed | — |
| [ADR-1577](1577-untrusted-input-boundary-and-injection-blocking.md) | Untrusted-input boundary + opt-in injection blocking | Proposed | — |
| [ADR-1606](1606-prohibition-enforcement-verify-seam.md) | prohibition-enforcement verify-time seam | Proposed | — |
| [ADR-1610](1610-workflow-agent-size-budget-ratchet.md) | workflow & agent size-budget ratchet (per-file byte baseline + tier hard caps) | Proposed | — |
