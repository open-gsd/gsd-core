...
两个钩子覆盖互补的攻击面。`gsd-prompt-guard.js` 监视*对规划产物的写入*——捕获注入的植入。`gsd-read-injection-scanner.js` 监视*任何文件的读取及远程获取*——捕获来自外部内容的注入摄取（依赖项的 README、第三方配置文件、用户提供的文档，或通过 WebFetch 或 WebSearch 获取的任何 URL）。研究代理中提示内的 `<security_context>` 边界提供了额外的隔离层：即使注入字符串到达代理，也会在结构上与指令区域分离。两者共同覆盖了摄取 → 存储 → 再读取的完整生命周期。
...
**提示注入防御无法消除的风险：** 足够有创意的、与已知模式不匹配的注入，或通过钩子未覆盖渠道到来的注入。此前未覆盖的渠道——子代理在浏览文档时读取的依赖项已发布 README 中的注入内容——现已由 `gsd-read-injection-scanner.js`（覆盖 WebFetch 和 WebSearch 输出）在摄取时扫描，并通过研究代理中的 `<security_context>` 边界在提示内进行结构隔离——但新颖的越狱方法和低信号注入仍可能无法被检测到。纵深防御意味着每一层使攻击更难——而非任何单一层使其不可能。
