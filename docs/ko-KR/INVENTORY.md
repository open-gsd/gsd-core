...
| `worktree-path-safety.md` | 실행기 경로 가드: 디스패치 시 삽입하는 루트 pin(단계 0p, #4254), 보호/기본 브랜치 HEAD(단계 0, #3819), cwd 드리프트(단계 0a, #3097), 절대 경로 경계(단계 0b, #3099). 루트 정규화와 고정된 상위 저장소의 서브모듈 규칙을 포함한다. |
