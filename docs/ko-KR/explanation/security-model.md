...
두 훅은 보완적인 표면을 커버한다. `gsd-prompt-guard.js`는 *계획 결과물에 대한 쓰기*를 감시한다 — 심어지는 인젝션을 잡는다. `gsd-read-injection-scanner.js`는 *모든 파일의 읽기 및 원격 패치*를 감시한다 — 외부 콘텐츠(의존성의 README, 타사 설정 파일, 사용자가 제공한 문서, 또는 WebFetch나 WebSearch를 통해 가져온 모든 URL)에서 수집되는 인젝션을 잡는다. 리서치 에이전트의 프롬프트 내 `<security_context>` 경계는 추가 격리 계층을 제공한다: 주입된 문자열이 에이전트에 도달하더라도 명령 영역으로부터 구조적으로 분리된다. 함께 수집 → 저장 → 재독 수명 주기를 괄호로 묶는다.
...
**프롬프트 인젝션 방어가 제거하지 않는 것:** 알려진 패턴과 일치하지 않는 충분히 창의적인 인젝션, 또는 훅이 커버하지 않는 채널을 통해 도달하는 인젝션. 이전에는 커버되지 않았던 서브에이전트가 문서를 탐색하면서 읽는 의존성의 공개된 README에 주입된 콘텐츠 채널은 이제 `gsd-read-injection-scanner.js`(WebFetch 및 WebSearch 출력 커버)에 의해 수집 시 스캔되고 리서치 에이전트의 `<security_context>` 경계에 의해 프롬프트 내에서 구조적으로 격리된다 — 그러나 새로운 탈옥이나 저신호 인젝션은 여전히 탐지되지 않을 수 있다. 심층 방어는 각 계층이 공격을 더 어렵게 만들지, 어떤 단일 계층이 불가능하게 만들지 않는다는 것을 의미한다.
