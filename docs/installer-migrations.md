...
| WorkBuddy | Skills in `skills/gsd-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through GSD | Global `WORKBUDDY_CONFIG_DIR` or `~/.workbuddy`; local `./.workbuddy` | GSD owns generated skills/agents/hooks and GSD settings entries only | WorkBuddy is built on the CodeBuddy Code core; install shape, hook event dialect, and slash-command/skill/agent surface mirror CodeBuddy — only the root path and the per-command `$ARGUMENTS` interpolation differ (`$ARGUMENTS` is preserved verbatim by WorkBuddy, not rewritten to `{{GSD_ARGS}}`). |
