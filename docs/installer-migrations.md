...
| Qoder | Claude Code-compatible skills in `skills/gsd-*/SKILL.md`; agents in `agents/`; hooks via `settings.json` (PreToolUse/PostToolUse/Stop) | Global `QODER_CONFIG_DIR` or `~/.qoder`; local `./.qoder` | GSD owns generated skills/agents/hooks and GSD settings entries only | [Qoder rules & AGENTS.md compatibility](https://docs.qoder.com/user-guide/rules#agents-md-compatibility); `settings.json` hooks follow Claude Code format; initial release — no migrations |
