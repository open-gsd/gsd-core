...
Synthesize patterns from steps 2-5 into structured JSON (entries keyed by `overview`, `data-flow`, `conventions`, and one `component:{Name}` per key component). Write `arch-decisions.json`. Then patch its timestamp:
```bash
