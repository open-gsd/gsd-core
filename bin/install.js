...
function selectRuntimesFromArgs(runtimeArgs) {
  if (runtimeArgs.includes('--all')) {
    return ['claude', 'kimi', 'kilo', 'opencode', 'gemini', 'codex', 'copilot', 'antigravity', 'cursor', 'windsurf', 'augment', 'trae', 'qwen', 'hermes', 'codebuddy', 'cline', 'qoder'];
  }
...
const banner = '\n' +
  ...
  '  development workflows for Claude Code, OpenCode, Gemini, Kimi CLI, Kilo, Codex, Copilot, Antigravity, Cursor, Windsurf, Augment, Trae, Qwen Code, Hermes Agent, Cline, CodeBuddy and Qoder.\n';
...
// Qoder converters — single implementations live in runtimeArtifactConversion (src/runtime-artifact-conversion.cts).
// Re-bound here so install.js call sites and exports continue to work unchanged.
...
// Qoder converters (convertClaudeToQoderMarkdown, convertClaudeAgentToQoderAgent,
// convertClaudeCommandToQoderSkill) live in runtimeArtifactConversion — bound at
// module top from gsd-core/bin/lib/runtime-artifact-conversion.cjs.
...
function install(isGlobal, runtime = 'claude', options = {}) {
  const { isOpencode, isKilo, isGemini, isCodex, isCopilot, isAntigravity, isCursor, isWindsurf, isAugment, isTrae, isQwen, isHermes, isCodebuddy, isCline, isKimi, isQoder } = runtimeFlags(runtime);
...
const allRuntimes = ['claude', 'antigravity', 'augment', 'cline', 'codebuddy', 'codex', 'copilot', 'cursor', 'gemini', 'hermes', 'kimi', 'kilo', 'opencode', 'qwen', 'trae', 'windsurf', 'qoder'];
...
function buildRuntimePromptText() {
  ...
  ${cyan}17${reset}) Qoder        ${dim}(~/.qoder)${reset}
  ${cyan}18${reset}) All
