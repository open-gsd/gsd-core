 ...
 *
 * Fix: write a .cmd shim using this function's typed IR. Its content is
 * `@ECHO OFF / @SETLOCAL / @"node.exe" "script.js" %*`.
 * cmd.exe executes
...
function buildCodexHookWindowsShimIR(scriptAbsPath, absoluteRunnerToken) {
  ...
  return {
    ...
    render: {
      // Use CRLF line endings for strict cmd.exe compatibility.
      // LF-only .cmd files work in modern Windows, but CRLF is canonical.
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  // GSD no longer uses that package, so this is legacy cleanup guidance only.
  // Guarded by GSD_SKIP_STALE_SDK_CHECK so CI/tests can silence it.
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  const { shouldInstallBanner, bannerCommand, pathSuggestionGlobalBin, pathSuggestionHomeDir } = bannerOpts;
  if (isGlobal && pathSuggestionGlobalBin) {
    maybeSuggestPathExport(pathSuggestionGlobalBin, pathSuggestionHomeDir || os.homedir());
  }
...
function maybeSuggestPathExport(globalBin, homeDir) {
  ...
  if (homePathCoveredByRc(globalBin, homeDir)) {
    console.log(`  ${yellow}⚠${reset} ${bold}gsd-tools${reset}'s directory is already on your PATH via an rc file entry — try reopening your shell (or ${cyan}source ~/.zshrc${reset}).`);
    return;
  }

  console.log('');
  console.log(`  ${yellow}⚠${reset} ${bold}${globalBin}${reset} is not on your PATH.`);
  console.log(`    Add it so your shell can find ${bold}gsd-tools${reset}:`);
...
/**
 * Resolve the bin directory that should contain the current `gsd-tools` binary
 * for a global npm-style install. Returns null when the installer is running
 * from a source checkout, npx temp package, or any layout that is not clearly a
 * global package installation.
 */
function resolveCurrentGlobalBinDir({ env = process.env, installDir = __dirname, platform = process.platform } = {}) {
  const explicitPrefix = env && env.npm_config_prefix;
  const npmGlobal = env && (env.npm_config_global === 'true' || env.npm_config_global === '1');
  if (explicitPrefix && npmGlobal) {
    return platform === 'win32' ? explicitPrefix : path.join(explicitPrefix, 'bin');
  }

  const normalized = path.resolve(installDir).replace(/\\/g, '/');
 ...
 *
 * Background: `@opengsd/gsd-sdk@0.1.0` was published once and is now retired.
 * GSD no longer depends on the standalone package or its `gsd-sdk` binary, but
 * leaving the package installed can confuse users inspecting global npm state.
 * Report only the known-bad legacy version and frame remediation as cleanup.
 *
...
function formatStaleStandaloneSdkWarning(info) {
  ...
  return [
    '⚠  Legacy standalone @opengsd/gsd-sdk@' + version + ' is installed globally.',
    '   GSD no longer uses the standalone SDK package or the gsd-sdk binary.',
    '   Remove it to avoid confusing old global npm state with the current gsd-tools runtime.',
    '',
    '   Remediation:',
...
function installAllRuntimes(runtimes, isGlobal, isInteractive) {
  ...
  const finalize = (shouldInstallStatusline, shouldInstallBanner) => {
    try {
      const pathSuggestionGlobalBin = isGlobal ? resolveCurrentGlobalBinDir() : null;
      let pathSuggestionEmitted = false;
      const printSummaries = () => {
        for (const result of results) {
          const useStatusline = statuslineRuntimes.includes(result.runtime) && shouldInstallStatusline;
          const pathSuggestionOpts = pathSuggestionGlobalBin && !pathSuggestionEmitted
            ? { pathSuggestionGlobalBin, pathSuggestionHomeDir: os.homedir() }
            : {};
          if (pathSuggestionGlobalBin && !pathSuggestionEmitted) pathSuggestionEmitted = true;
          finishInstall(
            result.settingsPath,
            result.settings,
